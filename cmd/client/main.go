...
import (
	...
	clienttools "github.com/itaraxa/shiny-broccoli/internal/clientTools"
...
func makeApp() *cli.App {
	return &cli.App{
		...
		Commands: []*cli.Command{
			{
				Name:    "test",
				Usage:   "start test SNMP cient",
				Aliases: []string{"t"},
				Action:  startTestClient,
				Flags: []cli.Flag{
...
func startClient(c *cli.Context) (err error) {
	...
	// TO-DO: добавить цикл с горутинами для запросов к разным клиентам
	...
	result, err := clienttools.GetData(params, OIDs)
	if err != nil {
		logger.WithFields(log.Fields{"error": err}).Error("error get data")
	}

	for _, item := range result {
		logger.Infof("OID = %s\tValue = %v", item.OID, item.Value)
	}

	return
}

func startTestClient(c *cli.Context) (err error) {
	logger := log.New()
	logger.SetFormatter(&log.TextFormatter{DisableLevelTruncation: true, FullTimestamp: true})
	logger.SetOutput(os.Stdout)
	switch strings.ToLower(c.String("logLevel")) {
